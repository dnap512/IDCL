...
def do_test(device, model,criterion, test_loader,used_model, save_dir, try_check):
    batch = 0
    test_accuracy = 0
    ...
    return str(round(test_accuracy/batch,4)), y_pred, y_true


def do_training(device, epochs, model,optimizer, criterion, 
                train_loader_stage1,train_loader_stage2, train_loader_stage3,
                val_loader_stage1, val_loader_stage2, val_loader_stage3,
                lr_scheduler
               ):
    val_losses = list()
    val_accuracies = list()
    ...
    for epoch in range(epochs):
        ...
        if epoch >= epoch_2 and avg_acc_val > best_acc:
